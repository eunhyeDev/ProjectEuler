...
/*
 * 2016.12.1 -3 (eunhyeDev)
 * 수평, 수직, 또는 대각선 방향으로 연속된 숫자 네 개의 곱 중 최대값은 얼마입니까?   
 */

public class problem11 {

	// 가장 큰 값, 가로 값, 세로 값, 오른쪽으로 대각선 값, 왼쪽으로 대각선 값.
	public static int maxValue = 0, WValue = 0, HValue = 0, RDValue = 0,
			LDValue = 0;
	...
	public static void main(String[] args) {
		...
		System.out.println(search(numberArr));
	}
	...
	public static int search(int[][] arr) {
		for (int i = 0; i < 17; i++) {
			for (int j = 0; j < 17; j++) {
				...
				// 가로
				WValue = arr[i][j] * arr[i][j + 1] * arr[i][j + 2]
						* arr[i][j + 3];
//				System.out.println("가로 : [" + i + "," + j + "][" + i + ","
//						+ (j + 1) + "][" + i + "," + (j + 2) + "][" + i + ","
//						+ (j + 3) + "], WValue = " + WValue + ", maxValue = "
//						+ maxValue);
				maxValue = Math.max(maxValue, WValue);

				// 세로
				HValue = arr[i][j] * arr[i + 1][j] * arr[i + 2][j]
						* arr[i+3][j];
//				System.out.println("세로 : [" + i + "," + j + "][" + (i + 1)
//						+ "," + j + "][" + (i + 2) + "," + j + "][" + (i + 3)
//						+ "," + j + "], HValue = " + HValue + ", maxValue = "
//						+ maxValue);
				maxValue = Math.max(maxValue, HValue);
				

				// 오른쪽 아래 대각선
				RDValue = arr[i][j] * arr[i + 1][j + 1] * arr[i + 2][j + 2]
						* arr[i + 3][j + 3];
//				System.out.println("오아대각선 : [" + i + "," + j + "][" + (i + 1)
//						+ "," + (j + 1) + "][" + (i + 2) + "," + (j + 2) + "]["
//						+ (i + 3) + "," + (j + 3) + "], RDValue = " + RDValue + ", maxValue = "
//						+ maxValue);
				maxValue = Math.max(maxValue, RDValue);
				

				if ((i > 2) && (j > 2)) {
					// 왼쪽 아래 대각선
					LDValue = arr[i][j] * arr[i - 1][j + 1] * arr[i - 2][j + 2]
							* arr[i - 3][j + 3];
					// System.out.println("왼아대각선 : [" + i + "," + j + "]["
					// + (i - 1) + "," + (j + 1) + "][" + (i - 2) + ","
					// + (j + 2) + "][" + (i - 3) + "," + (j + 3) +
					// "], LDValue = " + LDValue + ", maxValue = "
					//		+ maxValue);
					maxValue = Math.max(maxValue, LDValue);
				}
			}
		}
		return maxValue;
